...
- 计划状态：Tasks 1–8 完成；v1.2.0 已正式发布并部署到生产服务器
- 功能意图：项目所有者已于 2026-07-28 确认
- 版本目标：`1.2.0`
- 规划分支：`v1.2-planning`
- 实现分支：`v1.2-player-snapshot-save`
- 基线：正式版 `v1.1.0`
...
- [x] 主文件替换前临时文件已完整写入并刷新，支持时使用
      `ATOMIC_MOVE + REPLACE_EXISTING`。
- [x] 写入、刷新或替换任一步失败时，旧主文件和其他 UUID 文件保持不变。
- [x] 成功保存后只保留一个最近成功备份，不把半写临时文件当作备份。

**验证：**

- [x] 单元测试注入临时写入、刷新、备份和原子替换失败。
- [x] 每个失败点后重新加载仍得到最近一次成功配置。
- [x] `git diff --check`、JUnit 和 `./gradlew --no-daemon clean test build`
      通过。
...
- [x] 截断 JSON、非法字段或无效中继使整个玩家配置安全失败，不静默删条目。
- [x] 只有同一 UUID 的有效备份可恢复；损坏主文件被保留且当前命令不继续。
- [x] 玩家消息和日志只包含错误码、UUID/路径标识等必要元数据，不泄露配置
      正文。

**验证：**

- [x] 覆盖无备份、有效备份、损坏备份、恢复写入失败和两个 UUID 隔离。
- [x] 配置失败的 `fire` 保持 0 假人、0 使用、0 新增区块票据。
- [x] 恢复后下一次命令可读取最近成功配置，且原损坏副本仍存在。
...
- [x] 正常、失败和恢复路径都不产生半写主文件。
- [x] 玩家 UUID、主文件、备份和损坏副本严格隔离。
- [x] 旧 schema v1 和当前 schema v2 的迁移合同保持不变。
- [x] 完整 JUnit、服务端 GameTest 和 clean build 通过。
- [x] 项目所有者确认恢复策略后再进入玩家快照实现。
...
- [x] 玩家站立并看向有效目标时，简写保存的 dimension、spawn、lookAt 和
      target 与实际采集值一致。
- [x] 无方块命中、超出交互距离、路径/出生区块未加载或假人几何不可达时，
      返回既有稳定失败，不写配置。
- [x] 控制台/命令方块获得玩家限定错误；完整参数保存结果保持兼容。

**验证：**

- [x] 单元测试覆盖精确小数坐标、正负区块边界和视线命中点。
- [x] 失败前后配置文件、区块票据和世界实体数量不变。
- [x] 发布 JAR 的 schema 仍为 v2，未增加客户端入口或生产依赖。
...
- [x] `save <name>` 精确保存测试玩家的维度、脚部位置、命中点和目标指纹。
- [x] 保存时允许创建者站在采集位置；其未离开便 `fire` 时在生成前拒绝。
- [x] 玩家移开并准备本人珍珠后，快照保存的中继仍只使用一次并完整清理。

**验证：**

- [x] 覆盖控制台拒绝、同名覆盖、双 UUID 隔离和旧完整参数形式。
- [x] 覆盖无命中、目标改变、未加载路径和出生点被其他玩家占用。
- [x] 每条拒绝断言 0 假人、0 使用、0 新增票据和唯一错误消息。
...
- [x] 真实连接客户端站到夹具位置、看向目标并用简写保存成功。
- [x] 客户端移开后执行 `fire`，假人从保存位置使用一次，珍珠释放并传送。
- [x] 客户端可见消息、假人出现/退出、execution ID 和服务端终态一致。

**验证：**

- [x] 开发客户端三个场景各 20/20。
- [x] 生产发布 JAR 客户端三个场景各 20/20，`CodeSource` 仍来自发布 JAR。
- [x] 保存快照、假人可见和传送后截图及日志作为 CI 证据上传。
...
- [x] `save <name>` 是真实玩家可完成的最短保存路径。
- [x] 完整参数形式和 v1.1 配置继续兼容。
- [x] 所有保存拒绝路径没有配置、实体、交互或区块加载副作用。
- [x] 快照保存后的真实珍珠装置传送连续通过。
- [x] 代码审查没有高严重度问题。
...
- [x] 固定随机种子至少执行 1,000 次后 `activeCount == 0`。
- [x] spawn、aim、validate、use、cleanup 任一阶段抛错仍只有一个终态。
- [x] shutdown 与 cleanup 竞态不产生重复使用、重复上报或孤儿假人。

**验证：**

- [x] 每轮断言 `useCalls <= 1`、`terminalCount == 1`。
- [x] 失败种子和阶段名称进入断言消息，可稳定复现。
- [x] 隔离服重复启停后为 0 在线玩家、0 孤儿假人、0 新崩溃报告。
...
- [x] README、CHANGELOG、命令帮助、迁移说明和测试报告描述同一行为合同。
- [x] `v1.2.0-rc.1` JAR、提交、tag、SHA256SUMS 和 CI 证据一一对应。
- [x] 隔离服验证完成后停止，生产服 JAR、世界、配置和 screen 不变。

**验证：**

- [x] `clean test build`、开发客户端和生产 JAR 客户端门禁全部通过。
- [x] 隔离服覆盖正常保存、损坏恢复、无备份失败、重启和孤儿检查。
- [x] 下载 GitHub prerelease 资产并复核 SHA-256 与 `fabric.mod.json`。
...
- [x] 玩家无需输入维度或坐标即可可靠保存中继。
- [x] 快照保存准确复现当前玩家的有效位置和面向目标。
- [x] 完整参数命令与 v1.1 配置保持兼容。
- [x] 配置写入和恢复不静默丢失数据，不跨 UUID。
- [x] 所有失败路径不创建假人、不使用方块、不新增区块票据。
- [x] 1,000 次生命周期压力验证没有活动执行或孤儿假人。
- [x] JUnit、服务端 GameTest、客户端 GameTest、生产 JAR GameTest 和隔离服
      证据全部通过。
- [x] 文档、版本、提交、tag、JAR、SHA256SUMS 和 Release 一致。
- [x] 项目所有者审查并批准发布证据。
