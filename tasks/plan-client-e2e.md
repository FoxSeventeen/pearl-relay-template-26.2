...
- 实施状态：阶段 1–4、Checkpoint A–C 和修订后发布门槛审批均已完成
...
- [x] 原 16 项玩家测试分别映射到 JUnit、服务端 GameTest、客户端
      GameTest、隔离服务端或明确命名的残留人工检查。
- [x] Checkpoint C 通过后，真实传送和玩家可见生命周期不再标记为未自动化。
- [x] 残留人工检查只覆盖主观体验和指定整合包/环境兼容性。

**验证：**

- [x] 对照当前所有失败码审查覆盖矩阵。
- [x] 确认所有自动化证据都指向准确的候选版本哈希。
- [x] 文档明确规定修订后的门槛获批前不得发布稳定版 `v1.1.0`。
...
Task 7 已评估两个同时运行的原生客户端，相比现有双 UUID 服务端测试是否
能提供足够的额外证据。
...
当前四项均不满足。Pearl Relay 没有客户端入口、客户端状态或自定义网络
协议，隔离边界完全位于服务端命令源 UUID、配置、列表和补全。因此保留双
UUID 服务端测试作为隔离证据，不实施可选 Task 8；若前提变化则重新决策。
...
- [x] Pearl Relay 日常回归不再需要真人玩家。
- [x] 负责证明结果的末影珍珠由真实 Minecraft 客户端投掷。
- [x] Pearl Relay 使真实连接玩家完成传送。
- [x] 假玩家生命周期、玩家消息、日志和清理均自动验证。
- [x] 客户端测试进入 CI，并验证生产映射后的产物。
- [x] 现有测试继续通过，发布 JAR 仍不包含测试内容。
- [x] 验收文档准确区分自动化事实与残留主观检查。
- [x] 项目所有者于 2026-07-28 批准修订后的发布门槛。
