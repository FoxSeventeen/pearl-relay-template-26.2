...
- [x] 自动覆盖 `test/save/list/fire/remove` 的有效流程和玩家隔离。
- [x] 自动覆盖旧版配置重存、0/他人/1/多颗珍珠以及重复触发。
- [x] 每个拒绝断言错误码、假人数、交互次数和日志事件数量。

**验证：**

- [x] GameTest 使用真实命令分发器，不直接绕过命令层调用预检。
- [x] 接受执行恰好一条 `accepted`、一条同 ID `terminal`。
- [x] 预检拒绝恰好一条 `rejected`，没有对应假人或 `terminal`。
...
- [x] 所有卸载失败都在访问方块状态或扫描实体前返回。
- [x] 拒绝前后相关区块 readiness、票据和实体数量保持一致。
- [x] `getChunkNow` 可见但不可实体 tick 的邻区块仍然拒绝。

**验证：**

- [x] 单元测试覆盖所有区块边界和负坐标。
- [x] GameTest 或受控服务端测试证明目标卸载不会新增票据。
- [x] 失败报告明确区分真实服务端证据与合成 WorldView 证据。
...
- [x] 命令级有效流程和全部错误合同通过。
- [x] 每个拒绝均为 0 个新假人、0 次使用和 0 个新增区块票据。
- [x] 每个接受执行只有一次使用和一个终态。
- [x] 两个 UUID 的配置、补全和触发保持隔离。
...
- [x] Java 25 clean build、全部 JUnit/GameTest 和隔离服重启通过。
- [x] 代码审查没有高严重度问题。
- [x] 不适用：本次包含生产代码修复，因此不保留 `rc.1` 作为当前候选。
- [x] 若生产代码或玩家可见消息改变，升级 `1.1.0-rc.2`，重新构建、
      部署、校验和并运行全部无客户端矩阵。
- [x] 更新玩家实机清单，但仍不把稳定版标记为通过。
